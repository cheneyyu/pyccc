...
def train_protein_role_classifier(
    ...
    *,
    ...
    dump({"models": models, "roles": list(roles), "embedding_genes": proteins["gene_id"].astype(str).tolist()}, output / "role_model.joblib")
    card = {"model_type": "protein_role_classifier", "roles": list(roles), "metrics": metrics, "output_dir": str(output)}
