...
def build_predicted_lr_table(
    ...
    *,
    ...
    model_name: str = "universal_esmc300m_lgbm_pair_ranker_v0",
    model_version: str = "0",
    model_revision: str = "",
    ...
    out = pd.DataFrame(
        {
            ...
            "evidence": "ESMC-300M embedding + LightGBM role classifiers + LightGBM pair ranker",
            ...
            "ligand_secreted_like_score": selected.get("ligand_secreted_like_score", pd.Series([0.0] * len(selected))).astype(float),
            "ligand_membrane_like_score": selected.get("ligand_membrane_like_score", pd.Series([0.0] * len(selected))).astype(float),
            "receptor_secreted_like_score": selected.get("receptor_secreted_like_score", pd.Series([0.0] * len(selected))).astype(float),
            "receptor_membrane_like_score": selected.get("receptor_membrane_like_score", pd.Series([0.0] * len(selected))).astype(float),
